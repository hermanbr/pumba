...
// DelaySpec holds netem delay parameters for the combo command.
...
// LossSpec holds netem random loss parameters for the combo command.
type LossSpec struct {
	Percent     float64
	Correlation float64
}

// CorruptSpec holds netem corrupt parameters for the combo command.
type CorruptSpec struct {
	Percent     float64
	Correlation float64
}

// DuplicateSpec holds netem duplicate parameters for the combo command.
type DuplicateSpec struct {
	Percent     float64
	Correlation float64
}

// RateSpec holds netem rate parameters for the combo command.
...
// ComboSpec selects which netem effects to combine into a single qdisc.
...
func (s ComboSpec) validate() error {
...
func (s ComboSpec) effectChecks() []func() error {
...
// NewComboCommand create new netem combo command combining multiple netem
// effects (delay, loss, duplicate, corrupt, rate) in a single qdisc
func NewComboCommand(client netemClient,
	gp *chaos.GlobalParams,
	req *container.NetemRequest,
	limit int,
	spec ComboSpec, // enabled netem effects
) (chaos.Command, error) {
...
// Run netem combo command
func (n *comboCommand) Run(ctx context.Context, random bool) error {
